...
void *csv_realloc(void *ptr, size_t len)
{
    void *new_ptr = NULL;
...
int set_field(CSV_FIELD *field, char *text) {
    ...
    tmp = csv_realloc(field->text, field->length);
    if (tmp == NULL)
        return 1;
    field->text = tmp;
...
int append_field(CSV_BUFFER *buffer, size_t row) {
    ...
    temp_field = csv_realloc(buffer->field[row],
...
int append_row(CSV_BUFFER *buffer) {
    ...
    temp_width = csv_realloc(buffer->width, (buffer->rows + 1) *
            sizeof (size_t));
    if (temp_width != NULL) {
    ...
    temp_field = csv_realloc(buffer->field, (buffer->rows + 1) *
            sizeof (CSV_FIELD**));
    if (temp_field != NULL) {
...
int remove_last_field(CSV_BUFFER *buffer, size_t row) {
    ...
    if (row > buffer->rows - 1)
        ...
        temp_row = csv_realloc(buffer->field[row], entry
                * sizeof (CSV_FIELD*));
        if (temp_row != NULL)
...
int csv_save(char *file_name, CSV_BUFFER *buffer) {
    ...
    if (fileHandle2 == SYS_FS_HANDLE_INVALID) {
        ...
        SYS_CONSOLE_PRINT("\r\nSYS_OTA : reading file content before modification %s\r\n", read_buf);
        #endif
        //file_opened = true;
    ...
    for (i = 0; i < buffer->rows; i++) {
        for (j = 0; j < buffer->width[i]; j++) {
            //SYS_CONSOLE_PRINT("buffer->field[i][j]->text : %s\n",buffer->field[i][j]->text);
            chloc = strchr(buffer->field[i][j]->text, text_delim);
            if (chloc == NULL) {
                #ifdef OTA_CSV_DEBUG
                SYS_CONSOLE_PRINT("\r\nSYS_OTA_CSV : Searching field_delim : %s\r\n", buffer->field[i][j]->text);
                #endif
                chloc = strchr(buffer->field[i][j]->text, field_delim);
                //if (chloc != NULL)
                    #ifdef OTA_CSV_DEBUG
                    SYS_CONSOLE_PRINT("\r\nSYS_OTA_CSV : chloc is set\r\n");
                    #endif

            }
            if (chloc == NULL) {
                #ifdef OTA_CSV_DEBUG
                SYS_CONSOLE_PRINT("SYS_OTA_CSV : Searching new line : %s\r\n", buffer->field[i][j]->text);
                #endif
                chloc = strchr(buffer->field[i][j]->text, '\n');
                #ifdef OTA_CSV_DEBUG
                if (chloc != NULL)
                    SYS_CONSOLE_PRINT("SYS_OTA_CSV : chloc is set\r\n");
                #endif
            }
            ...
            if (chloc != NULL) {
                #ifdef OTA_CSV_DEBUG
                SYS_CONSOLE_PRINT("SYS_OTA_CSV : chloc != NULL \r\n");
                #endif    
                ...
                SYS_FS_FileSeek(fileHandle2, read_byte_cnt, SYS_FS_SEEK_SET);
                #ifdef OTA_CSV_DEBUG
                SYS_CONSOLE_PRINT("%c", text_delim);
                #endif
                //fputc(text_delim, fp);
                for (k = 0; k < buffer->field[i][j]->length - 1; k++) {
                    ...
                    SYS_FS_FileSeek(fileHandle2, read_byte_cnt, SYS_FS_SEEK_SET);
                    #ifdef OTA_CSV_DEBUG
                    SYS_CONSOLE_PRINT("%c", buffer->field[i][j]->text[k]);
                    #endif
                ...
                #ifdef OTA_CSV_DEBUG
                SYS_CONSOLE_PRINT("SYS_OTA_CSV : writing string : %s\r\n", buffer->field[i][j]->text, strlen(buffer->field[i][j]->text));
                #endif
...
int csv_clear_row(CSV_BUFFER *buffer, size_t row) {
    ...
    temp_row = csv_realloc(buffer->field[row], sizeof (CSV_FIELD*));
